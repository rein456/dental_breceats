...
function elementOuterSize(el, size, includeMargins) {
    const window2 = getWindow();
    {
        return (
            ...
            parseFloat(
                window2.getComputedStyle(el, null).getPropertyValue(size === "width" ? "margin-right" : "margin-top"),
            ) +
            parseFloat(
                window2.getComputedStyle(el, null).getPropertyValue(size === "width" ? "margin-left" : "margin-bottom"),
            )
        );
    }
}
    ...
    open(selectorValue) {
        if (bodyLockStatus) {
            ...
            if (this.targetOpen.element) {
                const codeVideo =
                    this.youTubeCode || this.targetOpen.element.getAttribute(`${this.options.youtubeAttribute}`);
                if (codeVideo) {
